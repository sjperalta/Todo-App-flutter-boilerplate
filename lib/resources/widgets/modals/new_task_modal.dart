...
class _NewTaskModalState extends State<NewTaskModal> {
  ...
  String _categoryId = 'personal'; // Default category

  @override
  void initState() {
    super.initState();
    // Set default category to first available if personal doesn't exist
    if (widget.categories.isNotEmpty) {
      final personalCategory = widget.categories.firstWhere(
        (cat) => cat.id == 'personal',
        orElse: () => widget.categories.first,
      );
      _categoryId = personalCategory.id;
    }
  }
  ...
  void _handleCreateTask() async {
    ...
    try {
      ...
      // Final validation of the created todo
      final todoErrors = todo.validate();
      ...
      if (mounted) {
        ErrorHandlingService.showSuccess(
          context,
          'Task "${todo.title}" created successfully',
        );
      }
    } catch (e) {
      setState(() {
      ...
      if (mounted) {
        ErrorHandlingService.showError(
          context,
          e,
          customMessage: 'Failed to create task. Please check your input and try again.',
        );
      }
    }
  }
